...
import zone.rong.loliasm.common.alternatecurrent.structure.queue.PowerQueue;
 ...
 * <br>
 * 2. Minimize the number of block updates emitted.
 * <br>
 * 3. Emit block updates in a deterministic, non-locational order,
 *    fixing bug MC-11193.
 *
 * <p>
 ...
 *
 * <p>
 * In addition to this, a wire emits 42 block updates each time it changes
 * its power level.
 *
 * <p>
 ...
 *   important, because calls to World.setIBlockState are even more
 ...
 * updates.
 * <br>
 * - Get rid of the 18 redundant block updates, so each wire only emits
 *   24 block updates whenever it changes its power level.
 * <br>
 * - Only emit block updates once a wire reaches its final power level,
 *   rather than at each intermediary stage. 
 * <br>
 * For an individual wire, these two optimizations are the best you can
 * do, but for an entire grid, you can do better!
 *
 * <p>
 * Since we calculate the power of the entire network, sending block
 * updates to the wires in it is redundant. Removing those updates can
 * reduce the number of block updates by up to 20%.
 *
 * <p>
 * 3.
 ...
 * base it on the EnumFacing of power flow. This part of the algorithm
 ...
 *
 * <p>
 * The idea is to determine the EnumFacing of power flow through a wire
 ...
 * it can be said that the EnumFacing of power flow through the wire is east. 
 *
 * <p>
 * We make the order of block updates to neighbors of a wire depend on what
 * is determined to be the EnumFacing of power flow. This not only removes
 * locationality entirely, it even removes EnumFacingality in a large
 * number of cases. Unlike in 'RedstoneWireTurbo', however, I have decided
 * to keep a EnumFacingal element in ambiguous cases, rather than to 
 ...
 * to neighboring wires in an order dependent on the EnumFacing of power
 * flow, we end up with a non-locational and largely non-EnumFacingal wire
 * update order.
 *
 * @author Space Walker
 */
public class WireHandler {
    ...
    public static class Facings {
        ...
        public static final EnumFacing[] ALL_ORDERED_UPDATES = { EnumFacing.WEST, EnumFacing.EAST, EnumFacing.NORTH, EnumFacing.SOUTH, EnumFacing.DOWN, EnumFacing.UP };
        ...
        // The cardinal Facings are ordered clockwise. This allows
        // for conversion between relative and absolute Facings
     ...
     * represents a cardinal EnumFacing, and when it is 'on', there is flow
     * in that EnumFacing.
     *
     * <p>
     ...
     *
     * <p>
     * If there is just 1 EnumFacing of incoming flow, that EnumFacing will
     * be the EnumFacing of outgoing flow.
     *
     * <p>
     * If there are 2 Facings of incoming flow, and these Facings are
     * not each other's opposites, the EnumFacing that is 'more clockwise'
     * will be the EnumFacing of outgoing flow. More precisely, the
     * EnumFacing that is 1 clockwise turn from the other is picked.
     *
     * <p>
     * If there are 3 Facings of incoming flow, the two opposing
     * Facings cancel each other out, and the remaining EnumFacing will
     * be the EnumFacing of outgoing flow.
     *
     * <p>
    ...
    /**
     * Update order of cardinal Facings. Given that the index is
     * to be considered the EnumFacing that is 'forward', the resulting
     * update order is { front, back, right, left }.
     */
    private static final int[][] CARDINAL_UPDATE_ORDERS = {
            { Facings.WEST , Facings.EAST , Facings.NORTH, Facings.SOUTH },
            { Facings.EAST , Facings.NORTH, Facings.SOUTH, Facings.WEST  },
            { Facings.NORTH, Facings.SOUTH, Facings.WEST , Facings.EAST  },
            { Facings.SOUTH, Facings.WEST , Facings.EAST , Facings.NORTH }
    };
    /**
     * The default update order of all Facings. It is equivalent to
     * the order of observer updates in vanilla Minecraft.
     */
    ...
    public WireHandler(WireBlock wireBlock, WorldAccess world) {
        ...
        this.nodes = new Long2ObjectOpenHashMap<>(); // Evaluate Object2Object or Long2Object
    ...
    /**
     * Retrieve the neighbor of a node in the given EnumFacing and
    ...
    /**
     * Check the IBlockState that occupies the given position. If it is
    ...
    private void invalidateNodes() {
        if (updatingPower && !nodes.isEmpty()) {
            for (Entry<Node> entry : nodes.long2ObjectEntrySet()) {
                entry.getValue().invalid = true;
            }
        }
    }
    ...
    private void findRoots(BlockPos pos, boolean checkNeighbors) {
        ...
        for (int iDir : DEFAULT_FULL_UPDATE_ORDER) {
            ...
            if (neighbor.isConductor()) {
                ...
                // solid blocks.
                findRedstoneAround(neighbor, Facings.iOpposite(iDir));
            } else if (world.emitsWeakPowerTo(neighbor.pos, neighbor.state, Facings.ALL[iDir])) {
                // Redstone components can also power multiple wires
                // directly.
                findRootsAroundRedstone(neighbor, Facings.iOpposite(iDir));
            }
        }
    }
    ...
    private void findRedstoneAround(Node node, int except) {
        for (int iDir : Facings.EXCEPT[except]) {
            Node neighbor = getNeighbor(node, iDir);
            if (world.emitsStrongPowerTo(neighbor.pos, neighbor.state, Facings.ALL[iDir])) {
    ...
    private void findRootsAroundRedstone(Node node, int except) {
        for (int iDir : Facings.EXCEPT[except]) {
            // Facings are backwards in Minecraft, so we must check
            // for power emitted in the opposite EnumFacing that we are
            // interested in.
            int iOpp = Facings.iOpposite(iDir);
            EnumFacing opp = Facings.ALL[iOpp];
            ...
            // this EnumFacing, move on to the next EnumFacing.
            if (!weak && !strong) {
                continue;
            }
            Node neighbor = getNeighbor(node, iDir);
    ...
    private int getExternalPower(WireNode wire) {
        int power = minPower;
        for (int iDir = 0; iDir < Facings.ALL.length; iDir++) {
            Node neighbor = getNeighbor(wire, iDir);
            if (neighbor.isWire()) {
                continue;
            }
            if (neighbor.isConductor()) {
                power = Math.max(power, getStrongPowerTo(neighbor, Facings.iOpposite(iDir)));
            }
            if (neighbor.isRedstoneComponent()) {
                power = Math.max(power, world.getWeakPowerFrom(neighbor.pos, neighbor.state, Facings.ALL[iDir]));
            }
            if (power >= maxPower) {
                return maxPower;
            }
    ...
    private int getStrongPowerTo(Node node, int except) {
        ...
        for (int iDir : Facings.EXCEPT[except]) {
            Node neighbor = getNeighbor(node, iDir);
            if (neighbor.isRedstoneComponent()) {
                power = Math.max(power, world.getStrongPowerFrom(neighbor.pos, neighbor.state, Facings.ALL[iDir]));
                if (power >= maxPower) {
                    return maxPower;
                }
            }
    ...
    private void findWirePower(WireNode wire, boolean ignoreNetwork) {
        for (int c = 0; c < wire.connections.count; c++) {
            ...
            if (!ignoreNetwork || !neighbor.inNetwork) {
                ...
                int iOpp = Facings.iOpposite(connection.iDir);
     ...
     * power change, emit observer and block updates, then queue
     * power changes for connected wires.
     */
...
//		profiler.swap("find powered wires");
        ...
        // block and observer updates are emitted, in case a different
        // network is updated that needs power changes.
//		profiler.swap("clear " + rootCount + " roots and network of " + network.size());
        rootCount = 0;
        network.clear();

        // Carry out the power changes and emit observer and block updates.
...
//			profiler.end();
     ...
     * to some backup value. This avoids EnumFacingality in redstone
     * grids.
     */
    private void addToNetwork(WireNode wire, int backupFlow) {
        ...
        // EnumFacingality, each wire node is given a 'backup' flow.
        // For roots, this is the determined flow of their
        // connections. For non-roots this is the EnumFacing from
     ...
     * EnumFacing of power flow through this wire. If that flow is
     * ambiguous, try to use a flow EnumFacing based on connections
    ...
    private void letPowerFlow() {
        ...
        while (!powerChanges.isEmpty()) {
            ...
            findPowerFlow(wire);
            if (wire.updateState()) {
                // If the wire was removed, observers were already updated.
                if (!wire.shouldBreak) {
    ...
    /**
     * Update observers around the given wire.
     */
    private void updateObservers(WireNode wire) {
        BlockPos wirePos = wire.pos;
        for (EnumFacing dir : Facings.ALL) {
            world.updateObserver(wirePos.offset(dir), wireBlock.asBlock(), wirePos);
        }
    }
     ...
     * <br>
     * 1. The EnumFacing of power flow through the wire is to be considered
     ...
     *    of a neighbor is encoded by the EnumFacing(s) of the step(s), e.g.
     ...
     *    absolute Facings when west is said to be 'forward':
     *    { west, east, north, south, down, up } - this is the order of observer
     *    updates.
     */
    private void updateNeighborBlocks(WireNode wire) {
        int iDir = wire.flowOut;
        EnumFacing forward   = Facings.HORIZONTAL[ iDir            ];
        EnumFacing rightward = Facings.HORIZONTAL[(iDir + 1) & 0b11];
        EnumFacing backward  = Facings.HORIZONTAL[(iDir + 2) & 0b11];
        EnumFacing leftward  = Facings.HORIZONTAL[(iDir + 3) & 0b11];
        EnumFacing downward  = EnumFacing.DOWN;
        EnumFacing upward    = EnumFacing.UP;
    ...
    private void updateNeighbor(BlockPos pos, BlockPos fromPos) {
        ...
        if (state != WorldAccess.AIR && !wireBlock.isOf(state)) {
